...
export class EscapeRoom {
    ...
    constructor(players: number, difficulty: number, theme: Theme) {
        ...
        [this.rooms, this.endPuzzle] = EscapeRoom.createRooms(totalTime, difficulty, this.puzzleManager, theme);
    ...
    static createRooms(totalTime: number, difficulty: number, manager: PuzzleManager, theme: Theme): [Room[], Puzzle] {
        let visited = new Set();
        let possible_locations: point[] = [[0,0]];
        let rooms: Room[] = [];
        let nrOfRooms: number = Math.floor(totalTime / 20);
        let graph = puzzleTreePopulator(totalTime, difficulty, manager, theme);
